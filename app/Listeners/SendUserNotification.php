...
class SendUserNotification
{
    ...
    public function handle(QuoteCreated $event)
    {
        ...
        $author_log = AuthorLog::where('author', $author)->first();
        if (!$author_log) {
            Mail::send('email.user_registration', ['name' => $author], function($message) use ($email, $author) {
                $message->from('admin@test.com', 'Admin');
                $message->to($email, $author);
                $message->subject('Please register!');
            });
        } else {
            Mail::send('email.user_notification', ['name' => $author], function($message) use ($email, $author) {
                $message->from('admin@test.com', 'Admin');
                $message->to($email, $author);
                $message->subject('Thank you for creating a Quote!');
            });
        }
    }
}
